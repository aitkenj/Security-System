...
void static commonInit(const uint8_t *cmdList) {
  ...
  // bit15      EUSCI_B_CTLW0_CKPH = 1; data shifts in on first edge, out on following edge
  // bit14      EUSCI_B_CTLW0_CKPL = 0; clock is low when inactive
  // bit13      EUSCI_A_CTLW0_MSB = 1; MSB first
  // bit12      EUSCI_B_CTLW0_SEVENBIT = 0; 8-bit data
  // bit11      EUSCI_A_CTLW0_MST = 1; master mode
